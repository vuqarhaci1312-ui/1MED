...
import { parseBlogYearMonthKey } from '../../utils/blogChronology';
...
const BlogWithSidebarContent = ({ sectionClass }: DataType) => {
    ...
    // Filter blogs by archive (ay/il) and category
    const filteredBlogs = BlogData.filter((blog) => {
        if (archiveFilter && parseBlogYearMonthKey(blog.date) !== archiveFilter) {
            return false;
        }
    ...
    const handlePageClick = (data: any) => {
        ...
        const categoryParam = categoryFilter !== 'Hamısı' ? `&category=${encodeURIComponent(categoryFilter)}` : '';
        const archiveParam = archiveFilter ? `&archive=${encodeURIComponent(archiveFilter)}` : '';
        navigate(`/blog-with-sidebar?page=${selectedPage}${categoryParam}${archiveParam}`);
    ...
    // Reset to page 1 when category or archive changes
    useEffect(() => {
        setCurrentPage(1);
    }, [categoryFilter, archiveFilter]);
